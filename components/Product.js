...
export default function Product(props){
    ...
    return(
        <div className={ styles.product }>
            <img srcSet={`/assets/products/${props.foto}.jpg`} alt="Nosso Produto"/>
            <h3>{produto.name}</h3>
            <small className={styles.oldPrice}>{produto.oldprice}</small>
            <p>R$ {produto.price}</p>
            <small className={styles.priceParcel}> ou 3x R$ {(produto.price / 3).toFixed(2)}</small>
